...
def subA(h, u, g, dt, dx):
    lambda_array, LgR = lambdfun(
        h[:-2], u[:-2], h[1:-1], u[1:-1]
    )  # LgR = 1 if lambda_{i-1} > lambda_{i}
    subDiag = (
        u[:-2] - lambda_array - LgR * (0.5 * np.sqrt(g / h[:-2]) * (h[:-2] - h[1:-1]))
    )
...
def diagA(h, u, g, dt, dx):
    ...
    diag = (
        lambda_m1
        - RgLm1 * (0.5 * np.sqrt(g / h[1:-1]) * (h[:-2] - h[1:-1]))
        + lambda_p1
        + LgRp1 * (0.5 * np.sqrt(g / h[1:-1]) * (h[1:-1] - h[2:]))
    )  # LgRp1 = 1 if lambda_{i} > lambda_{i+1}
    return diag * (-dt / (2 * dx))
...
def supA(h, u, g, dt, dx):
    ...
    supDiag = (
        u[2:] - lambda_array - RgL * (0.5 * np.sqrt(g / h[2:]) * (h[1:-1] - h[2:]))
    )
...
def diagB(h, u, g, dt, dx):
    ...
    lambda_p1, LgRp1 = lambdfun(h[1:-1], u[1:-1], h[2:], u[2:])
    RgLm1 = np.ones(np.shape(LgRm1)) - LgRm1  # RgLm1 = 1 if lambda_{i} > lambda_{i-1}
    diag = -RgLm1 * (np.sign(u[1:-1]) * (h[:-2] - h[1:-1])) + LgRp1 * (
        np.sign(u[1:-1])
    ) * (
        h[1:-1] - h[2:]
    )  # LgRp1 = 1 if lambda_{i} > lambda_{i+1}
...
def supB(h, u, g, dt, dx):
    ...
    supDiag = h[2:] - (np.ones(np.shape(LgR)) - LgR) * (np.sign(u[2:])) * (
        h[1:-1] - h[2:]
    )  # RgL = 1 if lambda_{i+1} > lambda_i
    return supDiag * (-dt / (2 * dx))
...
def subC(h, u, g, dt, dx):
    ...
    subDiag = (
        u[:-2] ** 2
        - g * h[:-2]
        - LgR * (0.5 * np.sqrt(g / h[:-2]) * (u[:-2] - u[1:-1]))
    )  # LgR = 1 if lambda_{i-1} > lambda_{i}
    return subDiag * (-dt / (2 * dx))
...
def diagC(h, u, g, k, eta, DZ, dt, dx):
    ...
    diag = (
        ...
        * (-dt / (2 * dx))
        * RgLm1
        * (0.5 * np.sqrt(g / h[1:-1]) * (u[:-2] - u[1:-1]))
        + LgRp1 * (0.5 * np.sqrt(g / h[1:-1]) * (u[1:-1] - u[2:]))
        - 2 * dx * g * DZ[1:-1]
...
def supC(h, u, g):
    ...
    RgL = np.ones(np.shape(LgR)) - LgR
    supDiag = (
        u[2:] ** 2 - g * h[2:] - RgL * (0.5 * np.sqrt(g / h[2:]) * (u[1:-1] - u[2:]))
    )  # RgL = 1 if lambda_{i+1} > lambda_i
    return supDiag
...
def supD(h, u, g):
    ...
    RgL = np.ones(np.shape(LgR)) - LgR
    supDiag = (
        2 * u[2:] * h[2:] + lambda_array - RgL * (np.sign(u[2:])) * (u[1:-1] - u[2:])
    )  # RgL = 1 if lambda_{i+1} > lambda_i
    return supDiag
...
def subAcons(h, q, g, dt, dx):
    ...
    subDiag = -lambda_array - LgR * (
        0.5 * np.sqrt(g / h[:-2]) - np.sign(q[:-2]) / h[:-2] ** 2
    ) * (h[:-2] - h[1:-1])
    return subDiag * (-dt / (2 * dx))
...
def diagAcons(h, q, g, dt, dx):
    ...
    diag = (
        lambda_m1
        - RgLm1
        * (0.5 * np.sqrt(g / h[1:-1]) - np.sign(q[1:-1]) / h[1:-1] ** 2)
        * (h[:-2] - h[1:-1])
        + lambda_p1
        + LgRp1
        * (0.5 * np.sqrt(g / h[1:-1]) - np.sign(q[1:-1]) / h[1:-1] ** 2)
...
def diagB(h, u, g, dt, dx):
    ...
    lambda_p1, LgRp1 = lambdfun(h[1:-1], u[1:-1], h[2:], u[2:])
    RgLm1 = np.ones(np.shape(LgRm1)) - LgRm1  # RgLm1 = 1 if lambda_{i} > lambda_{i-1}
    diag = -RgLm1 * (np.sign(q[1:-1]) / h[1:-1]) * (h[:-2] - h[1:-1]) + LgRp1 * (
        np.sign(q[1:-1]) / h[1:-1]
    ) * (
        h[1:-1] - h[2:]
    )  # LgRp1 = 1 if lambda_{i} > lambda_{i+1}
