...
def J_grad(K, hreference = href):
    ...
    return diradj.shallow_water_RSS_grad(D, g, T, h0, u0, N, LF_flux, dt, b, K,
                                         bcLref, bcR, bcL_A, bcR_A, hreference, J_function)
...
# ------------------------------------------------------------------------------
def J_grad_observed(K, A=amplitude, P=period,
                    idx_to_observe = np.arange(href.shape[0]), hreference = href):
...
# ------------------------------------------------------------------------------
def J_KAP(Kcoeff=Kref, A = amplitude, P = period,
...
# ------------------------------------------------------------------------------
def J_KAP_nograd(Kcoeff, A = amplitude, P = period,
                 idx_to_observe = np.arange(href.shape[0]),
                 hreference = href):
    """
    Computes RSS and its gradient wrt to K, with amplitude, period and observation index
    ...
    bcL = lambda h, hu, t: BCrand(h, hu, t, 'L',
                                  mean_h, A, P, phase)
    _, hcomputed, _, _ = diradj.shallow_water(D, g, T, h0, u0, N, LF_flux, dt, b, K,
                                              bcL, bcR)
...
def join_K_variables(design):
    ...
    if Kvar.shape[1] == 1:
        ...
        newdesign = [tuple([list(de[:-2]), de[-2], de[-1]]) for i, de in enumerate(design)]
        return newdesign


# ------------------------------------------------------------------------------
def J_KAP_array(KAP, idx_to_observe = idx_to_observe, hreference = href,
                parallel=False, ncores=None, adj_gradient=False):
    """
